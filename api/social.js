...
const handlers = {
   ...
   *
   * Requires INTERNAL_API_SECRET (x-internal-secret header) and:
   *   CLEANSOCIAL_OLD_USERNAME / CLEANSOCIAL_NEW_USERNAME env vars.
   */
  cleansocial: async (req, res) => {
    ...
    const oldTarget = normalizeUsername(process.env.CLEANSOCIAL_OLD_USERNAME);
    const myTrueNewUsername = normalizeUsername(process.env.CLEANSOCIAL_NEW_USERNAME);
    if (!oldTarget || !myTrueNewUsername) {
      return res.status(503).json({
        error:
          'Set CLEANSOCIAL_OLD_USERNAME and CLEANSOCIAL_NEW_USERNAME in the environment to run this action',
      });
    }
    if (oldTarget === myTrueNewUsername) {
      return res.status(400).json({ error: 'Old and new usernames must differ' });
    }
