...
class KitchenPersonalityAnalyzer:
    ...
    def analyze_user_personality(self, user_id):
        ...
        try:
            ...
            personality = {
                ...
                'experimental_score': float(round(experimental_score, 2)),
                'comfort_score': float(round(comfort_score, 2)),
                'cuisine_diversity': float(round(cuisine_analysis['diversity_score'], 2)),
                'skill_level': skill_level,
                'cooking_stats': {
                    'total_recipes': len(recipes),
                    'avg_rating': float(round(cooking_patterns['avg_rating'], 2)),
                    'avg_difficulty': float(round(cooking_style['avg_difficulty'], 2)),
                    'avg_cooking_time': float(round(cooking_patterns['avg_cooking_time'], 2)),
            ...
            self._save_personality_to_user(user_id, personality)
    ...
    def _save_personality_to_user(self, user_id, personality):
        """Save personality analysis to user's profile"""
        try:
            db.collection('users').document(user_id).update({
                'kitchen_personality': personality
            })
        except Exception as e:
            print(f"Error saving personality: {str(e)}")

# Cloud Function to analyze kitchen personality
@https_fn.on_request()
def analyze_kitchen_personality(req: https_fn.Request) -> https_fn.Response:
    """Analyze user's kitchen personality based on their cooking history"""
    if req.method != 'POST':
    ...
    try:
        ...
        if not user_id:
            return https_fn.Response('User ID is required', status=400)
        ...
        return https_fn.Response(json.dumps(personality), status=200)
        
    except Exception as e:
        return https_fn.Response(f'Error: {str(e)}', status=500)

# Triggered when a new recipe is created to update personality
# Note: Firestore triggers are commented out for now due to version compatibility
# @firestore_fn.on_document_created('recipe_posts/{recipe_id}')
def on_recipe_created_update_personality(event):
    """Update user's kitchen personality when they create a new recipe"""
    try:
        recipe_data = event.data.to_dict()
        user_id = recipe_data['username']
        
        # Analyze updated personality
        analyzer = KitchenPersonalityAnalyzer()
        personality = analyzer.analyze_user_personality(user_id)
        
        print(f"Updated personality for user {user_id}")
        
    except Exception as e:
        print(f"Error updating personality: {str(e)}")

# Triggered when a recipe is created in logs collection
# Note: Firestore triggers are commented out for now due to version compatibility
# @firestore_fn.on_document_created('logs/{log_id}')
def on_log_created_update_personality(event):
    """Update user's kitchen personality when they create a new log"""
    try:
        log_data = event.data.to_dict()
        user_id = log_data['username']
